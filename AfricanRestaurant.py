# African Restaurant with food menu project
...
class Banku(Restaurant):

    def __init__(self, kind, cost):
        super(Banku, self).__init__(kind, cost)
        self.condiments = []
...
class Combo(Restaurant):
    def __init__(self, kind, k, p, a, discount):
        ...
        self.cost = self.Banku.get_cost() + self.Drink.get_cost() + self.Side.get_cost() - self.discount

    def __str__(self):
        a = ""
        a = a + "combo: " + self.kind + "\n"
        a = a + str(self.Banku) + "\n" + str(self.Drink) + "\n" + str(self.Side) + "\n"
        a = a + "Cost of combo before discount: $" + str(
            self.Banku.get_cost() + self.Drink.get_cost() + self.Side.get_cost()) + "\n"
...
cost_of_banku = 15.00
condiments_for_banku = ["red-pepper", "shito", "okro", "rep-pepper and shito"]

types_of_drink = ["lemonade", "malt", "vita milk"]
size_of_drink = ["small", "medium", "large"]
cost_of_drink = [3.00, 4.00, 5.00]

cost_of_size = 10.00
sides_for_combo = ["Tilapia", "chicken", "goat"]

discount_for_combo = 2.00


def get_banku():
    k = Banku("Banku", cost_of_banku)
    f1 = input("Do you like to add any condiments for your banku? (y for yes) ")
    if f1.lower() == "y":
        for condiment in condiments_for_banku:
...
def get_combo():
    ...
    print("First, let's order the banku for your combo.")
    k = get_banku()
...
def take_order():
    ...
    print("Welcome to Golgi African Restaurant!")
...
print("Thank you for choosing Golgi African Restaurant!")
